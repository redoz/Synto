//HintName: Demo.M.TrailingReturn.g.cs
#nullable enable
...
partial class M
{
    public sealed record TrailingReturnMatch(ExpressionSyntax Result);
    public static TrailingReturnMatch? TrailingReturn(SyntaxNode node)
    {
        if (node is not BlockSyntax _blk)
            return null;
        TrailingReturnMatch? _TryAt(int _o)
        {
            ...
            if (_t2.AsToken().Text != ";")
                return null;
            return new TrailingReturnMatch(cap_result);
        }

        if (_blk.Statements.Count < 1)
            return null;
        int _o = _blk.Statements.Count - 1;
        if (_TryAt(_o)is { } _m)
            return _m;
        return null;
    }
