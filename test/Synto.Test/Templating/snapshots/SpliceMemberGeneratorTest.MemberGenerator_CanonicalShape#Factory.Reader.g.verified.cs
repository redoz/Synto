...
partial class Factory
{
    ...
    {
        global::System.Collections.Generic.IEnumerable<global::Microsoft.CodeAnalysis.CSharp.Syntax.MemberDeclarationSyntax> __spliceGenerator_Accessors_0()
        {
            foreach (var c in columns)
                yield return MethodDeclaration(
                             PredefinedType(Token(IntKeyword)), 
                             Identifier("Get" + c.Name)).AddModifiers(Token(PublicKeyword)).WithExpressionBody(
                             ArrowExpressionClause(
                             LiteralExpression(
                             NumericLiteralExpression, 
                             Literal(c.Ordinal)))).WithSemicolonToken(Token(SemicolonToken));
        }
        ...
        return ClassDeclaration(
                   ...
                   CollectionSyntaxExtensions.BuildList<MemberDeclarationSyntax>(CollectionSyntaxExtensions.ListSegment<MemberDeclarationSyntax>.Run(__spliceGenerator_Accessors_0())), 
