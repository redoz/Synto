...
/// <summary>
/// Per-region control-flow codegen for the staging emitter: builds the verbatim run+collect scaffold for a single
/// live control region (the <c>for</c>/<c>foreach</c>/<c>while</c>/<c>if</c> with its body partitioned into
/// root-renamed live runtime statements and quoted <c>__run.Add(&lt;island&gt;)</c> calls), and the staged-statement
/// predicates that classify a region/container statement as live runtime code vs an output-world quoted island.
/// Pure transform-internal helper driven via <see cref="StagedEmitContext"/>; nothing captured into pipeline state.
/// </summary>
internal static class StagedScaffoldBuilder
{
    public static bool TryBuildScaffold(StagedEmitContext context, StagedRegion region, string runName)
    {
    ...
    /// </summary>
    public static bool IsStagedStatement(SemanticModel semanticModel, StatementSyntax statement, HashSet<ISymbol> stagedSet)
    {
        switch (statement)
        {
