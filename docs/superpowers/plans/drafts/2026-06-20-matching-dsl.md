...
- **The per-green ff-push-to-main is suppressed.** github.md's standard `ready→implement` flow rebases each green commit onto `origin/main` and fast-forward-pushes `HEAD` straight to `main`. That path is **disabled** for this plan — it would land a stream of partial, mid-development commits *and* the one-way-door surface on `main`. This project does **not** practice full CI yet, so there is no green-gate-to-main to honor; do **not** route this plan through the standard main-pushing flow.
