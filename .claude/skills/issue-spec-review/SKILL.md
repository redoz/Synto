...
6. **Route** (github.md § Approval detection):
   ...
   - **LGTM** (non-`manual`) → **promote the spec, then make the skip-plan decision** (rigor.md, the **spec-review → plan?** knob). First promote the spec per github.md § Artifact storage & promotion (move the draft file → top-level `docs/superpowers/specs/{slug}.md`, edit the `## 📐 Spec` comment link to the promoted path, then **commit only the renamed paths**: `jj commit docs/superpowers/specs/{slug}.md docs/superpowers/specs/drafts/{slug}.md -m "docs(spec): promote {slug} (LGTM on #<n>)"` (jj tracks the rename from content similarity; **never** a bare `jj commit`/`jj squash` — github.md § Working-tree hygiene)); then `jj bookmark set "$(bash .claude/scripts/base-branch.sh)" -r @-` — advance B locally (no push; 'land on B' integration, github.md § Setting state; under SYNTO_FLOW_INTEGRATE=push, also `jj git push`-ed). The strict review team's LGTM **is** the gate. Then:
     - **skip-plan** (rigor heuristic met, not `manual`: the spec is fully diff-reviewable — no new feature folder/namespace, no cross-cutting pipeline change, no multi-step sequencing, no non-obvious TDD structure) → **derive a thin plan inline** (rigor.md § Skip-plan) and jump straight to `ready`:
       ...
       2. Commit **only that one file** (verify `jj status` shows nothing but it): `jj commit docs/superpowers/plans/{file} -m "docs(plan): derive thin one-shot plan for #<n> (skip-plan)"`. Then `jj bookmark set "$(bash .claude/scripts/base-branch.sh)" -r @-` — advance B locally (no push; 'land on B' integration, github.md § Setting state; under SYNTO_FLOW_INTEGRATE=push, also `jj git push`-ed).
